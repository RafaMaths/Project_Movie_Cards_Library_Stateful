...
class MovieLibrary extends Component {
  ...
  changes = (name, value) => {
    ...
    if (name === 'bookmarkedOnly') {
      const array = value ? moviesLibrary.filter((movie) => (movie.bookmarked
      === value))
        : (movieSet);
      this.setState({
        moviesLibrary: array,
    ...
    if (name === 'selectedGenre') {
      const array = value !== '' ? movieSet.filter((movie) => (movie.genre
        === value))
        : (movieSet);
      this.setState({
        moviesLibrary: array,
    ...
    if (name === 'searchText') {
      const array = value !== '' ? movieSet.filter((movie) => (movie
        ...
        || (movie.storyline.includes(value))) : (movieSet);
      this.setState({
        moviesLibrary: array,
      });
    }
